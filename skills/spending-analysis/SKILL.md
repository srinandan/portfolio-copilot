---
name: spending-analysis
description: >-
  Analyzes Chase transaction data via BigQuery — categorizes spend, flags
  anomalies, answers trend questions via natural-language-to-SQL. Use when
  the user asks about spending patterns, budget categories, or wants a
  trend explained (e.g. "why did dining spend jump in June").
metadata:
  version: "0.1.0"
  status: draft
---

# spending-analysis

## Purpose

Analyzes Chase transaction data in BigQuery. Categorizes spend, flags anomalies, answers trend questions via NL-to-SQL.

## Inputs

TODO: define inputs (schema / source)

## Outputs

TODO: define outputs (schema)

## Tools / permissions required

TODO: list MCP servers, APIs, or read/write scopes this skill needs

## Registry metadata

- Registered as: `projects/{project}/locations/{location}/skills/private-spending-analysis`
- Skill revision: 0.1.0 (draft — not yet registered)
- Approval scope: TODO

## Notes

Stub generated from the functional spec (docs/spec/01-functional.md).
Fill in before registering with the Agent Registry.
