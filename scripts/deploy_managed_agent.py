...
def store_secret(project: str, secret_id: str, secret_value: str) -> None:
    ...
    try:
        ...
        logger.info("google-cloud-secret-manager not installed; falling back to gcloud secrets CLI")
    ...
    try:
        ...
        logger.warning(f"Failed to store secret via gcloud: {e}")
...
def find_existing_managed_agent(project: str, location: str, display_name: str) -> Optional[str]:
    ...
    try:
        ...
        logger.warning(f"Could not list agents via gcloud alpha agents list: {e}")

    return None


def provision_managed_agent(project: str, location: str, display_name: str) -> str:
    ...
    2. Creates a new Managed Agent via gcloud alpha agents create.
    ...
    try:
        logger.info(f"Creating Managed Agent '{display_name}' via gcloud alpha agents create...")
        proc = subprocess.run(cmd, capture_output=True, text=True, check=True)
        ...
        logger.warning(f"Creation via gcloud alpha agents create failed or unconfigured: {e}")

    # Fallback to deterministic resource name for local/preview sandbox environments
    fallback_id = f"projects/{project}/locations/{location}/agents/{display_name}"
    logger.info(f"Using standard target agent ID: {fallback_id}")
    return fallback_id
...
def deploy_managed_agent(project: str | None, location: str, display_name: str):
    ...
    try:
        # 1. Provision or resolve server-assigned agent ID
        server_agent_id = provision_managed_agent(project, location, display_name)
        ...
        if project_number_proc.returncode == 0:
            ...
            orchestrator_sa = f"portfolio-copilot-orchestrator@{project}.iam.gserviceaccount.com"
            grant_iam_role(project, f"serviceAccount:{orchestrator_sa}", "roles/secretmanager.secretAccessor")
