...
| [`skills/equity-research/`](../skills/equity-research/) | [`equity_research.evalset.json`](../skills/equity-research/equity_research.evalset.json) | • Two-stage DCF valuation and verdict calculation (undervalued / fairly_valued / overvalued)<br>• Missing/non-positive free cash flow handling without hallucination (`valuation_verdict: unknown`)<br>• Strict isolation and public data only (zero private financial data access)<br>• Missing price quote handling yielding unknown verdict<br>• Mandatory investment-advice disclaimers |
| [`skills/suitability/`](../skills/suitability/) | [`suitability.evalset.json`](../skills/suitability/suitability.evalset.json) | • Buy recommendation for undervalued security when headroom exists under concentration limit and equity sleeve is under target<br>• Hold recommendation when security is at concentration limit ceiling<br>• Trim recommendation for overvalued security currently held<br>• Avoid recommendation for excluded tickers or prohibited sectors<br>• Advisory-only invariant (never drafts or executes trades) |
...
To ensure runtime governance and skill filtering work end-to-end against live Google Cloud Agent Registry and Agent Platform Sessions services, the repository includes an automated revocation demo (`scripts/demo_live_revocation.py`):
