...
| Market & fundamentals data (read-only) | SEC EDGAR (free XBRL fundamentals) + Alpaca market quotes, behind a pluggable `FundamentalsProvider` with a TTL cache | [0027](../adr/0027-equity-research-and-suitability-advisory-analysis.md) |
...
**SEC EDGAR + Alpaca (read-only external data)** — public company fundamentals (as-reported XBRL from `data.sec.gov`) and market quotes, consumed by the `equity-research` skill for DCF valuation. Accessed through a pluggable `FundamentalsProvider` (`executors/sec_edgar.py`, `executors/market_data.py`) with a TTL cache (`data/fundamentals_cache.py`) to respect SEC fair-use, and a deterministic offline mock so the path runs without network or credentials. Unlike BigQuery/Firestore this is not user data, so it is safe to cache and share. The advisory analysis is also exposed synchronously at `POST /v1/analysis/equity` (bypassing the streaming planner) for the Portfolio-view "Research a stock" panel. See [ADR-0027](../adr/0027-equity-research-and-suitability-advisory-analysis.md).
