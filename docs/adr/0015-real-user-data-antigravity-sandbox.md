...
In the Portfolio Copilot personal-use demo, the assistant operates on the project owner's personal financial data (own checking transaction records, paper-trading portfolio holdings, and Investment Policy Statement). Restricting skills to synthetic data limits the utility of onboarding interviews, spending analytics, and drift evaluations.

## Decision
Personal financial data (own checking transactions, own paper-trade holdings, own IPS) may be routed through Antigravity's sandbox as tool inputs and Managed Agent invocation inputs, subject to strict boundary constraints:
