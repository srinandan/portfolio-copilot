...
class ResearchBrief(BaseModel):
    ...
    summary: str = Field(
        description="Objective synthesis of market context, performance drivers, and analyst consensus."
    )
    sources: list[str] = Field(default_factory=list, description="List of cited public URLs and reference sources.")
    confidence: ConfidenceLevel = Field(
        description="Confidence rating (high, medium, low) indicating source certainty."
    )
