...
class UserProfile(BaseModel):
    ...
    family_members: List[FamilyMember] = Field(
        default_factory=list, description="List of family members and dependents."
    )
    ...
    monthly_housing_payment_usd: Optional[float] = Field(
        default=None, ge=0.0, description="Monthly rent or mortgage payment in USD."
    )
    risk_tolerance_notes: Optional[str] = Field(
        default=None, description="User notes regarding risk tolerance and volatility comfort."
    )
    financial_goals_notes: Optional[str] = Field(
        default=None, description="User notes regarding financial objectives and milestones."
    )
