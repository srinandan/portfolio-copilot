...
def test_lifespan_initializes_and_closes_bigquery_mcp_toolset():
    from src.orchestrator import server
    ...
    with (
        ...
        patch(
            "src.orchestrator.data.bigquery_mcp.list_available_mcp_tools",
            new_callable=AsyncMock,
            return_value=["list_datasets", "execute_query"],
        ),
        patch("src.orchestrator.server.SessionManager"),
        patch("src.orchestrator.server.Runner"),
    ):
        with TestClient(server.app):
            assert server.state.bigquery_mcp_toolset == mock_toolset

        # After lifespan exit, toolset must be closed and cleared
        mock_toolset.close.assert_awaited_once()
        assert server.state.bigquery_mcp_toolset is None


def test_lifespan_bigquery_mcp_discovery_failure_handled_gracefully():
    from src.orchestrator import server
    ...
    with (
        ...
        patch(
            "src.orchestrator.data.bigquery_mcp.get_bigquery_mcp_toolset_from_registry",
            side_effect=RuntimeError("BigQuery MCP unreachable"),
        ),
        patch("src.orchestrator.server.SessionManager"),
        patch("src.orchestrator.server.Runner"),
    ):
...
def test_lifespan_bigquery_mcp_timeout_handled_gracefully():
    import asyncio
