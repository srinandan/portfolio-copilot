...
async def _run_gate(node_input):
    mock_ctx = MagicMock()
    result = None
...
@pytest.mark.asyncio
async def test_gate_executes_on_approved_no_verdict(sample_proposed_action):
...
@pytest.mark.asyncio
async def test_gate_handles_alpaca_execution_error(sample_proposed_action):
    mock_executor = MagicMock()
...
@pytest.mark.asyncio
async def test_gate_handles_alpaca_credentials_missing(sample_proposed_action):
    mock_executor = MagicMock()
...
@pytest.mark.asyncio
async def test_gate_passes_approving_user_id_to_audit(sample_proposed_action):
    mock_executor = MagicMock()
    mock_executor.submit_order.return_value = ExecutionResult(
