...
type ProposedAction struct {
	...
	Status                 string          `json:"status"` // drafted, reviewed_pass, reviewed_fail, pending_approval, approved, rejected, executed, failed
	CreatedAt              time.Time       `json:"created_at"`
}

// IsTrade returns true if the action type is a trade.
func (pa *ProposedAction) IsTrade() bool {
	return pa.Type == "trade"
}

// Validate returns true if the ProposedAction meets basic validation rules.
func (pa *ProposedAction) Validate() bool {
	if pa.ActionID == "" || pa.SessionID == "" || pa.Type == "" || pa.Ticker == "" || pa.Side == "" {
		return false
	}
	if pa.Quantity <= 0 || pa.Rationale == "" || pa.Status == "" || pa.CreatedAt.IsZero() {
		return false
	}
