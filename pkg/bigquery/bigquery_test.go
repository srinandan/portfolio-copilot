...
func TestPrepareSecureSQL(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "Basic valid query with checking_transactions and explicit columns",
			generatedSQL: "SELECT user_id, amount FROM checking_transactions",
			expectError:  false,
			checkQuery: func(t *testing.T, secureSQL string, params map[string]interface{}) {
				if !strings.Contains(secureSQL, "portfolio_copilot.checking_transactions") {
					t.Errorf("Expected scoped table name in query, got: %s", secureSQL)
		...
		},
		{
			...
			generatedSQL: "-- Calculate total spent\n/* Block comment */\nSELECT category, SUM(amount) FROM checking_transactions GROUP BY category;",
			expectError:  false,
			checkQuery: func(t *testing.T, secureSQL string, params map[string]interface{}) {
				if !strings.HasPrefix(secureSQL, "WITH checking_transactions AS") {
		...
		},
		{
			...
			generatedSQL: "SELECT category, SUM(amount) FROM checking_transactions GROUP BY category LIMIT 10",
			expectError:  false,
			checkQuery: func(t *testing.T, secureSQL string, params map[string]interface{}) {
		...
		},
		{
			...
			generatedSQL:  "DELETE FROM checking_transactions WHERE amount > 100",
			expectError:   true,
			expectedError: "only SELECT queries are allowed",
		},
		{
			name:          "Write query rejected - MERGE",
			generatedSQL:  "MERGE portfolio_copilot.checking_transactions T USING (SELECT '@attacker' AS user_id) S ON TRUE WHEN MATCHED THEN UPDATE SET T.user_id = S.user_id",
			expectError:   true,
			expectedError: "only SELECT queries are allowed",
		},
		{
			name:          "Write query rejected - EXPORT DATA",
			generatedSQL:  "EXPORT DATA OPTIONS(uri='gs://bucket/*.csv', format='CSV') AS SELECT * FROM checking_transactions",
			expectError:   true,
			expectedError: "only SELECT queries are allowed",
		},
		{
			name:          "Write query rejected - LOAD DATA",
			generatedSQL:  "LOAD DATA INTO checking_transactions FROM FILES(format='CSV', uris=['gs://bucket/data.csv'])",
			expectError:   true,
			expectedError: "only SELECT queries are allowed",
		},
		{
			name:          "Write query rejected - EXECUTE IMMEDIATE",
			generatedSQL:  "EXECUTE IMMEDIATE 'INSERT INTO checking_transactions VALUES (1, 2)'",
			expectError:   true,
			expectedError: "only SELECT queries are allowed",
		},
		{
		...
		},
		{
			...
			generatedSQL:  "SELECT * FROM checking_transactions; DROP TABLE checking_transactions;",
			expectError:   true,
		...
		},
		{
			...
			generatedSQL: "SELECT update_date FROM checking_transactions",
			expectError:  false,
			checkQuery: func(t *testing.T, secureSQL string, params map[string]interface{}) {
		...
		{
			name:         "Query targeting full path portfolio_copilot.checking_transactions is rewritten to CTE",
			generatedSQL: "SELECT * FROM portfolio_copilot.checking_transactions WHERE amount > 50",
			expectError:  false,
			checkQuery: func(t *testing.T, secureSQL string, params map[string]interface{}) {
				// The main SELECT after CTE should reference checking_transactions (pointing to the CTE)
				if !strings.Contains(secureSQL, "SELECT * FROM checking_transactions WHERE amount > 50") {
					t.Errorf("Expected main query to reference CTE checking_transactions, got: %s", secureSQL)
				}
			},
		},
		{
			// Regression test: BigQuery CTEs only shadow unqualified names. A query
			// using `otherproj.otherds.checking_transactions` would otherwise skip the
			// user_id scoping entirely.
			name:         "Query with foreign project qualifier is stripped to CTE",
			generatedSQL: "SELECT * FROM `otherproj.otherds.checking_transactions` WHERE amount > 50",
			expectError:  false,
			checkQuery: func(t *testing.T, secureSQL string, params map[string]interface{}) {
				...
				if !strings.Contains(secureSQL, "FROM checking_transactions WHERE amount > 50") {
					t.Errorf("Expected main query to reference bare checking_transactions, got: %s", secureSQL)
				}
			},
		},
		{
			name:         "Query with bare project.dataset.checking_transactions qualifier is stripped",
			generatedSQL: "SELECT * FROM myproj.myds.checking_transactions WHERE amount > 50",
			expectError:  false,
			checkQuery: func(t *testing.T, secureSQL string, params map[string]interface{}) {
		...
		},
		{
			...
			name:         "Query joining foreign checking_transactions is stripped so both sides hit CTE",
			generatedSQL: "SELECT c1.amount FROM checking_transactions c1 JOIN `otherproj.otherds.checking_transactions` c2 ON c1.txn_id = c2.txn_id",
			expectError:  false,
			checkQuery: func(t *testing.T, secureSQL string, params map[string]interface{}) {
